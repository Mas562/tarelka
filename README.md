...
**macOS 14+ · SwiftUI · Русский интерфейс · Исходники 2.4.0**

[Скачать для Mac](https://github.com/Mas562/tarelka/releases/latest) · [Начать](#начать) · [Возможности](#возможности) · [Скриншоты](#скриншоты) · [Apple Watch](docs/AppleWatch.md) · [Для разработчиков](#для-разработчиков)
...
| **Строка меню** | Значок рядом с часами открывает дневной баланс. Оттуда можно добавить напиток, повторить недавнее блюдо или открыть нужный раздел. |
| **Виджет macOS** | Компактный баланс калорий и БЖУ на рабочем столе; нажатие открывает «Мой день». Виджет пересчитывает текущий день по локальному дневнику, даже если основное окно закрыто. |
...
Откройте [последний релиз](https://github.com/Mas562/tarelka/releases/latest) и скачайте архив `Tarelka-…-arm64.zip`. Распакуйте его и перенесите «Тарелка.app» в «Программы». В релизе также есть файл контрольных сумм `SHA256SUMS.txt`. Возможности версии 2.4.0 появятся в готовой загрузке после публикации соответствующего релиза; сейчас их можно собрать из исходников.

Архив предназначен для Apple Silicon и macOS 14+. Сборка имеет локальную ad-hoc подпись, без Developer ID и нотарификации Apple; macOS может ограничить запуск загруженного приложения. Модели Ollama скачиваются отдельно. Ниже — альтернативный путь сборки из исходников.
...
После сборки распакуйте `dist/Тарелка.zip` вне папки iCloud Drive, например в «Программы», и откройте приложение. Копия `dist/Тарелка.app` также создаётся для локальной проверки, но Finder в папке iCloud Drive может добавить ей метаданные и нарушить локальную подпись.
...
### Строка меню и виджет

Когда «Тарелка» запущена, нажмите значок вилки и ножа в строке меню Mac. Мини-окно показывает остаток калорий и БЖУ. Кнопка **Напиток** открывает форму напитка, **Новое блюдо** — ввод блюда, а нажатие на недавнюю запись добавляет такую же порцию в сегодняшний дневник.

Чтобы поставить виджет на рабочий стол, нажмите правой кнопкой на рабочем столе → **Изменить виджеты**, найдите **Тарелка — баланс дня** и выберите маленький или средний размер. Нажатие на виджет открывает раздел **Мой день**. Виджет читает только локальные файлы дневника на этом Mac; данные в облако не отправляются. После изменения дневника приложение просит macOS обновить виджет, а при смене дня он пересчитывается сам. Частоту показа обновлений определяет macOS.
...
| Виджет | Читает эти же два файла локально; отдельная облачная копия не создаётся |
...
└── TarelkaWidget/          # Виджет баланса для рабочего стола
Tests/
...
В исходниках 2.4.0 пройдены **99 тестов в 18 наборах**. Набор проверок охватывает арифметику порций и БЖУ, напитки, импорт активности, недельные отчёты, обработку ответов моделей, сохранение диктовки после пауз, календарные переходы и баланс виджета. Тесты используют контролируемые данные и не заменяют проверку качества распознавания реальных блюд.
...
| Виджет не появился в галерее | Откройте установленную «Тарелку» хотя бы раз, затем заново откройте галерею виджетов. Проверьте, что приложение распаковано из архива и лежит вне iCloud Drive. |
